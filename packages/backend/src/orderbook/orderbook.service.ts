...
export class OrderbookService {
  ...
  private async validateOrder(order: Order): Promise<void> {
    ...
    if (order.side === 'BUY') {
      ...
      const requiredAmount = new Decimal(order.priceLimit || 0)
        .mul(order.qtyUnits)
        .mul(order.bondId) // This should be fraction value calculation
        .toNumber();
  ...
  private async matchOrder(order: Order): Promise<{ trades: Trade[] }> {
    ...
    const oppositeOrders = await this.getOrderBookSide(order.bondId, oppositeSide);
  ...
  private async updatePortfolios(
    ...
    const tradeAmount = price * qty; // This should be fraction value calculation
  ...
  private async addToOrderbook(order: Order): Promise<void> {
    const key = `orderbook:${order.bondId}:${order.side}`;
    const score = order.side === 'BUY' ? -order.priceLimit : order.priceLimit; // Negative for buy to get descending order
    
    await this.redis.zadd(key, score, order.id);
  }

  private async removeFromOrderbook(order: Order): Promise<void> {
    const key = `orderbook:${order.bondId}:${order.side}`;
    await this.redis.zrem(key, order.id);
  }

  private async getOrderBookSide(bondId: number, side: 'BUY' | 'SELL'): Promise<OrderBookLevel[]> {
    const key = `orderbook:${bondId}:${side}`;
    const orderIds = await this.redis.zrange(key, 0, -1);
